...
This directory contains the Docker setup for the Recaply application, including all core services for development, observability, and monitoring.
...
### Core Application
- **API (FastAPI)**: Main backend application running on Uvicorn
...
Before starting the system, create required environment files:
...
```bash
docker compose up -d api nginx pgvector qdrant
```
